...
class RRL:
    def __init__(self, dim_list, device_id, use_not=False, is_rank0=False, log_file=None, writer=None, left=None,
        ...
        self.gamma = gamma
        # self.best_f1 = -1.
        self.best_f1 = 1e20 # 其实是best_mse, 越小越好
        self.best_loss = 1e20
    ...
    def train_model(self, data_loader=None, valid_loader=None, epoch=50, lr=0.01, lr_decay_epoch=100,
        ...
        #     # print("X: "+str(X.type)+" y: "+str(y.type))
        #     # if torch.isnan(X).all() == False:
        #     #     print("nan in X!!!!!!!!!!!!!")
        ...
        for epo in range(epoch):
            ...
            for X, y in data_loader:
                ...
                # print("Train: X: "+str(X)+" y: "+str(y))

                # trainable softmax temperature
                # print("self.......: "+str(self.net.forward(X)))
                ...
                # print("y: "+str(y.shape))
                # print("y_bar: "+str(y_bar.shape))
                # print("y_arg: " + str(y_arg.shape))
                ...
                if str(ba_loss_rrl) == 'nan':
                    torch.set_printoptions(threshold=np.inf)
                    print(str(X))
                    # print(str(y))
                    print("total: "+str(total_run))
                    print("ba_cnt: loss is nan: "+str(ba_cnt))
                    p = True
                    break
                else:
                    print("ba_cnt: ok")
                    total_run += 1
                print("ba_loss_rrl: ~~~~~~~~~~~ "+str(ba_loss_rrl))
                # if ba_loss_rrl == float('nan'):
                #     print("ba_cnt: loss is nan: "+str(ba_cnt))
                #     break
                ...
                if self.is_rank0 and (cnt % (TEST_CNT_MOD * (1 if self.save_best else 10)) == 0):
                    if valid_loader is not None:
                        mse, r2 = self.test(test_loader=valid_loader, set_name='Validation')
                    else: # use the data_loader as the valid loader
                        mse, r2 = self.test(test_loader=data_loader, set_name='Training')
                    ...
                    if self.save_best and (mse < self.best_f1 or (np.abs(mse - self.best_f1) < 1e-10 and self.best_loss > epoch_loss_rrl)):
                        self.best_f1 = mse
                        self.best_loss = epoch_loss_rrl
                        self.save_model()
                    ...
                    if self.writer is not None:
                        self.writer.add_scalar('MSE_RRL', mse, cnt // TEST_CNT_MOD)
                        self.writer.add_scalar('R2_RRL', r2, cnt // TEST_CNT_MOD)

            print("ba_cnt:"+str(ba_cnt))
            if p == True:
                break
    ...
    def test(self, test_loader=None, set_name='Validation'):
        ...
        # print("y_true2: "+str(y_true))
        # y_true = np.argmax(y_true, axis=1)
        # print("y_true3: "+str(y_true))
        data_num = y_true.shape[0]
        # print("data_num: "+str(data_num))

        slice_step = data_num // 40 if data_num >= 40 else 1
        logging.info('y_true: {} {}'.format(y_true.shape, y_true[:: slice_step]))
        ...
        # y_pred_b = y_pred_b * y_std + y_mean


        # mse = np.mean((y_true - y_pred_b) ** 2)
        mse = mean_squared_error(y_true, y_pred_b) # mse
        # mse = mean_absolute_error(y_true, y_pred_b) # rmse
        ...
        logging.info('-' * 60)
        logging.info('On {} Set:\n\tMSE of RRL  Model: {}'
                     '\n\tR2 of RRL  Model: {}'.format(set_name, mse, r2))
        # logging.info('On {} Set:\nPerformance of  RRL Model: \n{}\n{}'.format(
        #     set_name,
        #     metrics.confusion_matrix(y_true, y_pred_b),
        #     metrics.classification_report(y_true, y_pred_b)))
        logging.info('-' * 60)
